...
def averageMatrix(MatrixList):
	qList = np.zeros([len(MatrixList),4])
	for n in range(len(MatrixList)):
		qList[n] = RotToQuat(MatrixList[n])
	return QuatToR(averageQuat(qList))
